-- position : position en WGS84 (SRID 4326)
...
INNER JOIN public.etats_inventaires etat 
ON LOWER(public.unaccent (etat.libelle)) = LOWER(public.unaccent (inv.etat))

INNER JOIN public.types_inventaires typ 
ON LOWER(typ.libelle) = CASE
